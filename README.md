# Hi ![](https://user-images.githubusercontent.com/18350557/176309783-0785949b-9127-417c-8b55-ab5a4333674e.gif) My name is ARMAN

## **Developer | CS Student | Code Enthusiast**

I am a Computer Science and Mathematics enthusiast passionate about low-level programming, algorithmic problem-solving, and clean web development.

I enjoy exploring complex mathematical logic—from vector calculus to system design—and building functional, well-structured projects from the ground up.

My programming journey is grounded in **C programming, C++, JavaScript, HTML, CSS**, and backend technologies. Currently, I am learning the **MERN Stack** and strengthening my development skills with **Node.js, Express.js, MongoDB, and APIs**.

* 🌍 I'm based in **India**
* 🧠 I'm currently learning **MERN Stack**
* 💻 I'm focused on **Node.js, Express.js, MongoDB & JavaScript**
* 🔧 I enjoy building **real-world web projects**
* 🧩 I'm interested in **low-level programming and algorithms**
* 📐 I'm passionate about **Computer Science & Mathematics**
* 💬 Ask me about **C, C++, JavaScript, Node.js, Express.js & Web Development**

### Skills

<p align="left">

<a href="https://docs.microsoft.com/en-us/cpp/?view=msvc-170" target="_blank" rel="noreferrer">
<img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/c-colored.svg" alt="C" title="C" width="36" height="36" />
</a>

<a href="https://docs.microsoft.com/en-us/cpp/?view=msvc-170" target="_blank" rel="noreferrer">
<img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/cplusplus-colored.svg" alt="C++" title="C++" width="36" height="36" />
</a>

<a href="https://docs.microsoft.com/en-us/dotnet/csharp/" target="_blank" rel="noreferrer">
<img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/csharp-colored.svg" alt="C#" title="C#" width="36" height="36" />
</a>

<a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript" target="_blank" rel="noreferrer">
<img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/javascript-colored.svg" alt="JavaScript" title="JavaScript" width="36" height="36" />
</a>

<a href="https://code.visualstudio.com/" target="_blank" rel="noreferrer">
<img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/visualstudiocode-colored.svg" alt="VS Code" title="VS Code" width="36" height="36" />
</a>

<a href="https://developer.mozilla.org/en-US/docs/Glossary/HTML5" target="_blank" rel="noreferrer">
<img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/html5-colored.svg" alt="HTML5" title="HTML5" width="36" height="36" />
</a>

<a href="https://www.w3.org/TR/CSS/#css" target="_blank" rel="noreferrer">
<img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/css3-colored.svg" alt="CSS3" title="CSS3" width="36" height="36" />
</a>

<a href="https://getbootstrap.com/" target="_blank" rel="noreferrer">
<img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/bootstrap-colored.svg" alt="Bootstrap" title="Bootstrap" width="36" height="36" />
</a>

<a href="https://nodejs.org/en/" target="_blank" rel="noreferrer">
<img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/nodejs-colored.svg" alt="NodeJS" title="NodeJS" width="36" height="36" />
</a>

<a href="https://expressjs.com/" target="_blank" rel="noreferrer">
<img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/express-colored-dark.svg" alt="Express" title="Express" width="36" height="36" />
</a>

<a href="https://www.mysql.com/" target="_blank" rel="noreferrer">
<img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/mysql-colored.svg" alt="MySQL" title="MySQL" width="36" height="36" />
</a>

<a href="https://www.mongodb.com/" target="_blank" rel="noreferrer">
<img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/mongodb-colored.svg" alt="MongoDB" title="MongoDB" width="36" height="36" />
</a>

<a href="https://cloud.google.com/" target="_blank" rel="noreferrer">
<img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/googlecloud-colored.svg" alt="Google Cloud" title="Google Cloud" width="36" height="36" />
</a>

</p>

### What I Work With

* 💻 **Programming:** C, C++, C#
* 🌐 **Frontend:** HTML, CSS, JavaScript, Bootstrap
* ⚙️ **Backend:** Node.js, Express.js
* 🗄️ **Databases:** MongoDB, MySQL
* 🔌 **APIs:** JavaScript APIs & Backend APIs
* 🧠 **Core Concepts:** Data Structures, Algorithms & Problem Solving
* 📐 **Mathematics:** Mathematical Logic, Calculus & Vector Calculus
* 🛠️ **Tools:** VS Code, npm, Git & GitHub

### Projects

🚀 **Weather App**

A JavaScript-based weather application using API integration to fetch and display weather information.

🎵 **Spotify Clone**

A frontend music-player project built using HTML, CSS and JavaScript with audio playback functionality.

📝 **To-Do Application**

A JavaScript DOM-based task management application with adding, deleting and completing tasks.

💬 **Chat Application**

A backend application built using Node.js, Express.js, EJS, MongoDB and Mongoose.

🏠 **REST / Listing Application**

A backend project using Express.js, MongoDB and Mongoose for managing listings and RESTful routes.

🛡️ **AI-Based Fake Identity & Document Screening System**

A Smart India Hackathon project focused on AI-assisted document screening, OCR extraction, document validation, tampering detection and face verification for identity documents.

### Socials

<p align="left">

<a href="https://www.github.com/Arman-alk" target="_blank" rel="noreferrer">

<picture>

<source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/socials/github-dark.svg" />

<source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/socials/github.svg" />

<img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/socials/github.svg" width="32" height="32" alt="GitHub" title="GitHub" />

</picture>

</a>

</p>

### Badges

<b>My GitHub Stats</b>

<p align="center">

<img src="https://github-readme-stats.vercel.app/api?username=Arman-alk&show_icons=true&bg_color=000000&title_color=facc15&text_color=ffffff&icon_color=facc15&hide_border=true" />

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Arman-alk&layout=compact&bg_color=000000&title_color=facc15&text_color=ffffff&hide_border=true" />

</p>

### 🔥 GitHub Streak

<p align="center">

<img src="https://streak-stats.demolab.com/?user=Arman-alk&theme=dark&hide_border=true" alt="GitHub Streak" />

</p>

